---
description: art-paper /art-mark-read — record human-read signal for one or more citation keys
model: sonnet
---
...
Implementation: invoke `scripts/art_paper_mark_read.py <citation_key>... --passport-path <path>` via Bash. The CLI handles validation (citation_key must exist in `literature_corpus[]`; on miss emit `[art-paper-MARK-READ ERROR: citation_key '<slug>' not in literature_corpus[]]` and refuse to write), 4 fail-fast environment checks (no active passport / passport not found / parent unreadable / read-log unwritable), and append-only write per §3.6 firm rule 3.
