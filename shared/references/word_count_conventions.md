...
Most publishers use a whitespace-equivalent algorithm in practice but state different conventions in their author guidelines. The 3–5% buffer rule (below) makes art-paper pipelines robust to the convention divergence without per-publisher adapters.

### Common publisher families

| Publisher / venue | Stated algorithm | Practical effect |
|---|---|---|
| Springer / Nature | whitespace-equivalent for abstract, no explicit definition | matches art-paper canonical |
| IEEE | "approximately N words" — whitespace-equivalent in their submission tool | matches art-paper canonical |
| ACM | whitespace-equivalent in CCS submission | matches art-paper canonical |
| Elsevier (Cell, Lancet families) | whitespace-equivalent in EM submission | matches art-paper canonical |
| Wiley | whitespace-equivalent | matches art-paper canonical |
| arXiv | no hard cap on abstract; advisory ~250 words | guideline only |
| ICLR / NeurIPS / ACL workshop tracks | hyphenated-as-1 in their LaTeX `\abstract{}` counter | matches art-paper canonical |
...
If the publisher's algorithm is ambiguous, default to art-paper canonical (whitespace-split) plus the 3–5% buffer.

---

## The 3–5% buffer rule

art-paper pipelines reserve **3–5% below the publisher's stated hard cap** as buffer. For a 250-word abstract:
...
Mandarin, Japanese, Korean, Thai, and Lao do not delimit words with whitespace. art-paper canonical algorithm produces meaningless totals on these texts.
...
- art-paper feedback memory `feedback_word_count_convention_mismatch.md` — original empirical observation of whitespace-split vs. hyphenated-as-1 differential (~12 words on a 250-word abstract).
