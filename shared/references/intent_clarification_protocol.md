...
**Spec:** v3.9.2 hot-fix per `ref/academic-research-skills/docs/design/2026-05-18-ars-v3.9.2-phase-boundary-spec.md`
...
When art-paper receives input that does not unambiguously identify the intended workflow, the main session **clarifies before dispatching any agent**. Auto-routing to a single-phase agent on cross-phase materials caused #133 (silent scope inflation).
...
|---|---|---|
| User invokes `/art-*` slash command | Explicit | Route directly to named skill; no clarification |
...
| 4 — Composition | Full draft, abstract, report body | `phase4_*/`; `draft.md`; `abstract.md` (or 5b for art-paper) |
...
(a) **Full pipeline** — go from materials through to a complete deliverable (research + write + review + revise). Use `art-pipeline` orchestrator (`/art-full`).
(b) **<phase-specific workflow 1>** — <one-line description>. Use `<skill>` (`/art-<mode>`).
(c) **<phase-specific workflow 2>** — <one-line description>. Use `<skill>` (`/art-<mode>`).
...
- `[direct-mode] /art-lit-review on these papers` → dispatches art-paper lit-review mode on the provided papers, no clarification
...
>
> (a) **Full paper from existing scaffolding** — use your abstract as paper seed, expand literature into bibliography, draft full paper, then review/revise. Use `art-pipeline` (`/art-full`).
> (b) **Lit-review only** — produce an annotated bibliography matrix from the literature; don't touch your abstract. Use `art-paper` (`/art-lit-review`).
> (c) **Quality-check the abstract against the literature** — verify abstract claims are supported by the cited sources. Use `art-reviewer` (`/art-citation-check`).
> (d) **Something else** — let me know what you're trying to do.
>
> Pick a-d, or describe the target deliverable. If you want me to dispatch a specific agent directly without this clarification, prefix your message with `[direct-mode]`.
...
**Main session response:** Routes directly to `art-paper /art-lit-review` on the provided PDFs. No clarification fires (explicit intent overrides materials check).
...
>
> (a) **I have an idea but no clear research question** — Socratic mode to clarify direction. Use `art-inquiry` (`/art-plan`).
> (b) **I have a question and need to do the research** — full research pipeline. Use `art-pipeline` (`/art-full`).
> (c) **I have research notes and need to write the paper** — drafting pipeline. Use `art-paper`.
> (d) **I have a draft and want feedback** — peer-review simulation. Use `art-reviewer` (`/art-review` equivalent).
>
...
- **Within a skill that has already been routed.** If `art-pipeline` is already running and the orchestrator is dispatching Phase 3 → Phase 4 internally, this protocol does not fire on every internal dispatch. It governs the **entry point** of a art-paper session, not in-pipeline transitions.
