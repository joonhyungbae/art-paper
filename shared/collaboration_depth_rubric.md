...
**Canonical location**: `shared/collaboration_depth_rubric.md` in `joonhyungbae/art-paper`. External consumers should reference by stable URL; do not vendor (bump the `rubric_version` field on any modification).
...
This rubric operationalizes their framework as a post-hoc observer signal for Claude Code agents. It is used by `art-pipeline/agents/collaboration_depth_agent.md` to score dialogue logs and produce **advisory, non-blocking** feedback to the user on how their collaboration pattern is shaping learning depth.
...
5. **Cross-model divergence flag**: if the consumer agent runs cross-model (e.g., via `CRS_CROSS_MODEL`), any dimension score disagreement > 2 points between models must be flagged in the output.
...
- Replace AI self-reflection protocols (e.g., `art-pipeline` Stage 6's six-dimension Collaboration Quality Evaluation, which is AI-about-itself; this rubric is AI-about-user-collaboration).
