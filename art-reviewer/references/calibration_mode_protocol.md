...
A single LLM reviewer produces an absolute 0-100 rubric score, but that score is weakly interpretable without knowing the reviewer's error profile. Two reviewers could give the same art paper a 65, yet one might systematically over-score conceptually thin but spectacular work and the other might systematically under-score quiet, conceptually rigorous work. Absolute scores don't reveal this.
...
Translation for art-paper: **our art-papers jury has an error profile too, and we do not currently measure it.** Calibration mode closes that gap. It does not try to make the jury perfect; it makes its imperfections legible. (Note: the Lu 2026 baselines below are from ML peer review; treat them as a methodological reference for the *measurement*, not as art-paper acceptance norms.)
...
1. **Gold-standard set**: 5-20 papers the user has labelled with known outcomes. Minimum 5; recommended 10-15. Each entry:
   ...
   - Venue context (program / venue, ambition tier)
   - Optional: human reviewer scores for comparison

2. **Domain specification**: the user's target medium/program, used to seed `field_analyst_agent`. Calibration for "generative / computational art" is not valid for "bio-art / art-science" — error profiles are medium-specific.
...
**Cross-model verification**: In calibration mode, `CRS_CROSS_MODEL` is **default-on** rather than opt-in. At least one of the 5 runs should use a different model family if available, to avoid single-model blind spots. If no cross-model is configured, emit a warning and run all 5 on the primary model.
...
<table of 5 art-paper review dimensions with mean absolute calibration error>

## Systematic biases detected
<natural-language narrative identifying patterns, e.g.
 "Reviewer tends to over-score Artistic Significance on spectacular but conceptually thin work"
 "Reviewer under-scores Conceptual Contribution on quiet, text-light work by ~8 points vs ground truth"
>
...
1. **Median instead of mean for rubric scores**: mean is vulnerable to single-run outliers (e.g., a run that hallucinates an un-anchored capability flaw); median is robust.
2. **Fresh context per run**: Lu 2026 allowed within-session memory across runs. art-paper uses fresh context to prevent cascading errors from a single run's misreading.
...
| `realization-focus` | Calibration should ideally be run with realization-heavy gold papers if this mode is the user's target. |
...
- **Activation**: opt-in only. User invokes `calibration` mode explicitly. art-paper does not auto-calibrate on first use in a new domain.
...
- **Shipped gold sets**: not planned for v3.2. Users bring their own gold set. Shipping a built-in art-medium gold set was considered and rejected to avoid domain-coverage bias and staleness.
...
- art-paper `shared/cross_model_verification.md` — cross-model reviewer integration.
- art-paper `art-reviewer/references/quality_rubrics.md` — scoring rubric definitions.
