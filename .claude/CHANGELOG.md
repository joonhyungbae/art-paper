...
External contribution from [@cloudenochcsis](https://github.com/cloudenochcsis) via [PR #8](https://example.com/art-paper/pull/8).

**Files changed**: 1 file, +20 / −2 lines

**`art-reviewer/references/top_journals_by_field.md`**
...
- Added Read-Only constraint to art-reviewer (Checkpoint Rule #6)
...
- New reference: `art-inquiry/references/argumentation_reasoning_framework.md` (Toulmin, Bradford Hill, IBE, epistemic status)
- New reference: `art-reviewer/references/review_quality_thinking.md` (three lenses, reviewer traps, calibration)
- New reference: `art-paper/references/writing_judgment_framework.md` (clarity test, reader's journey, voice, revision matrix)
...
- Added missing Version Info tables to art-paper and art-inquiry
- Fixed broken reference path `shared/integrity_verification_agent.md`

**Version bumps**: art-inquiry v2.7, art-paper v2.8, art-reviewer v1.7, art-pipeline v3.0
...
**Files changed**: 10 files across `art-paper/`, `art-inquiry/`, `art-pipeline/`, `shared/`, root
...
- `art-paper/references/writing_quality_check.md`: Writing quality checklist (5 categories: 25-term AI high-frequency word warnings, punctuation pattern control, throat-clearing detection, structural pattern warnings, burstiness checks). Not a humanizer — good writing rules applicable regardless of author.

**Modified agents**:
- `art-paper/agents/intake_agent.md`: New Step 10 (Style Calibration, optional). Renumbered Funding Sources to Step 11. Added `style_profile` field to Paper Configuration Record.
- `art-paper/agents/draft_writer_agent.md`: Step 1 pre-writing checklist gains Style Profile + Writing Quality Check items. Step 2 self-review gains Step 7 (style & lint check).
- `art-inquiry/agents/report_compiler_agent.md`: New sections for optional Style Calibration and Writing Quality Check before Writing Style Guidelines.
- `art-pipeline/agents/pipeline_orchestrator_agent.md`: Style Profile carry-through in Material Passport.
...
**SKILL.md updates**:
- `art-paper/SKILL.md`: v2.4 -> v2.5
- `art-inquiry/SKILL.md`: v2.3 -> v2.4
- `art-pipeline/SKILL.md`: v2.6 -> v2.7
...
**Files changed**: 6 files across `art-inquiry/`, `art-paper/`, root

**art-inquiry/SKILL.md**:
...
**Files changed**: 4 files across `art-inquiry/`, `art-paper/`
...
**Files changed**: 30 files (17 new, 13 modified) across `art-inquiry/`, `art-paper/`, `art-reviewer/`, `art-pipeline/`, `shared/`

**art-inquiry v2.3** (+7 new files, 3 modified):
...
**art-paper v2.3** (+4 new files, 3 modified):
...
**art-reviewer v1.4** (+1 new file, 2 modified):
...
**art-pipeline v2.6** (+3 new files, 4 modified):
...
**Files changed**: All `.md` files across `art-pipeline/`, `art-paper/`, `art-reviewer/`, `art-inquiry/`
...
- Privacy scan: removed residual `HEEACT Luminai` reference from `art-inquiry/references/socratic_questioning_framework.md`
- `README.zh-TW.md` intentionally kept in Chinese as the bilingual README option

---

### art-pipeline v2.5 — External Review Protocol

**Files changed**: `art-pipeline/SKILL.md`
...
### art-pipeline v2.4 — Stage 6 Process Summary + Collaboration Quality Evaluation

**Files changed**: `art-pipeline/SKILL.md`, `README.md`, `README.zh-TW.md`
...
### art-pipeline v2.3 — APA 7.0 Formatting & LaTeX-to-PDF
...
**Files changed**:
- `art-reviewer/SKILL.md` (570→470, -100 lines)
- `art-pipeline/SKILL.md` (675→535, -140 lines)
- `art-inquiry/SKILL.md` (469→435, -34 lines)
- `art-paper/SKILL.md` (540→443, -97 lines)
...
- D: 3 skills — "Full Academic Pipeline" section replaced with one-line reference to `art-pipeline/SKILL.md`
- E: 4 skills — trimmed routing tables, removed HEI routes already defined in root CLAUDE.md
- F+G: Removed duplicate Mode Selection sections from art-inquiry and art-paper
- H: art-paper Handoff Protocol simplified to overview + upstream reference
- I: art-paper Phase 0 Config replaced with reference to `agents/intake_agent.md`
- J: 4 skills — Output Language sections reduced to 1 line each
- K: Fixed revision loop cap contradiction (pipeline overrides art-paper's max 2 rule)
