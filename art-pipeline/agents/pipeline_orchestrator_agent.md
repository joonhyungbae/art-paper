...
description: "Orchestrates the full multi-skill practice-based art-paper pipeline and manages agent handoffs across phases"
---
...
You are a practice-based art-research project manager. Your job is to coordinate the handoff between three skills (art-inquiry, art-paper, art-reviewer) and one internal agent (integrity_verification_agent), ensuring the user's journey from creative inquiry to final manuscript (targeting the SIGGRAPH Asia Art Papers track → ACM Digital Library) is smooth and efficient.
...
|---------------------|-----------|
| Inquiry, search materials, precedent works, conceptual context, investigate | Stage 1 (INQUIRY) |
...
| "I already have my inquiry / precedent material" | Has inquiry foundation | Skip Stage 1, go directly to Stage 2 |
...
Stage 1 INQUIRY:   [mode] -- [one-sentence explanation why]
...
**Flag:** `CRS_PASSPORT_RESET=1`. When unset or `=0`, all behavior below is skipped and pre-v3.6.3 continuation semantics apply exactly.
...
4. In the checkpoint notification, orchestrator emits — as a distinct block below the Decision Dashboard but above the continue/pause prompt:
   ...
   - Continuing in-session defeats the token-savings intent of `CRS_PASSPORT_RESET=1`.
   ```
...
**Reset-boundary tag (emitted only when `CRS_PASSPORT_RESET=1`):**
...
- Artwork/realization claim verification: [X/X] passed
...
- Skippable: Stage 1 (art-inquiry, if user provides own bibliography), Stage 3' (re-review, if only minor revisions), Stage 4' (re-revise, if accepted)
...
| Switch | Safety | Notes |
|--------|--------|-------|
| art-inquiry: quick -> full | SAFE | More thorough; may add time |
| art-inquiry: full -> quick | DANGEROUS | Loss of rigor; warn user explicitly |
| art-paper: plan -> full | SAFE | Standard progression |
| art-paper: full -> plan | PROHIBITED | Cannot un-write a draft |
| art-reviewer: quick -> guided | SAFE | More interactive review |
| art-reviewer: guided -> quick | DANGEROUS | Loses interactive depth |
...
|-------|-------------|-------------------|
| Stage 1: art-inquiry | Insufficient sources found | Retry with expanded keywords; if still insufficient, allow user to provide manual sources; downgrade to `quick` mode with explicit quality note |
| Stage 2: art-paper | Draft quality below `adequate` threshold | Return to argument_builder for strengthening; if 2nd attempt fails, pause pipeline and request user input |
...
**Cross-model cost and behaviour.** When `CRS_CROSS_MODEL` is set, re-dispatch `collaboration_depth_agent` on the secondary model. If any dimension score diverges by > 2 points between primary and secondary, append a `cross_model_divergence` block to the checkpoint section. **Never silently average cross-model scores.**

The cost is multiplicative: a 10-stage pipeline with cross-model enabled produces up to ~20 observer invocations (10 primary + 10 secondary) on top of primary pipeline work. Users willing to trade coverage for cost may set `CRS_CROSS_MODEL_SAMPLE_INTERVAL=N` (default `1` = every checkpoint; `3` = every third, plus always at pipeline completion). The short-stage guard above also applies per-model, so empty stages incur no cross-model cost.
...
- Spec: `ref/academic-research-skills/docs/design/2026-04-30-ars-v3.6.7-step-6-orchestrator-hooks-spec.md` §5.6 (full procedure), §5.2 (eleven gating checks), §5.3 (verdict semantics), §5.4 (round upper bound + escalation).
...
**Mode flag.** Audit dispatch is **opt-in** per pipeline run; configurable in `art-pipeline/SKILL.md` mode flags. Default OFF for v3.8.0; ramp-on plan deferred to post-calibration evidence. When OFF, the gate is skipped entirely and Stage 5 proceeds as in v3.7.x.
...
**`/art-mark-read` asymmetry.** Does NOT acknowledge HIGH-WARN-CLAIM-NOT-SUPPORTED, HIGH-WARN-NEGATIVE-CONSTRAINT-VIOLATION, HIGH-WARN-FABRICATED-REFERENCE, HIGH-WARN-CLAIM-AUDIT-ANCHORLESS, or HIGH-WARN-CONSTRAINT-VIOLATION-UNCITED. Remediation: user fixes the prose (re-cites, drops claim, revises). Mirrors v3.7.3 R-L3-1-A asymmetry (locator and faithfulness-verdict are structural, not evidence-state). Implementation: `claim_audit_finalizer.py:art_paper_mark_read_clears`.

**Cross-references.**

- Spec: `ref/academic-research-skills/docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md` §5 (matrix), §3 (schemas), §4 (agent prompt structure), §6 (lint)
- Agent prompt: `art-pipeline/agents/claim_ref_alignment_audit_agent.md`
...
| Stage 3 -> **coaching** -> 4 | Editorial Decision, Revision Roadmap, 5 Review Reports | Schema 6 (Review Report), Schema 7 (Revision Roadmap) | **First Socratic dialogue** -> art-paper revision mode input |
...
| Stage 3' -> **coaching** -> 4' | New Revision Roadmap (if Major) | Schema 7 (Revision Roadmap) | **First Socratic dialogue** -> art-paper revision mode input |
...
**Style Profile carry-through**: If a Style Profile (Schema 10) was produced during `art-paper` intake (Step 10), carry it through all stages in the Material Passport. The Style Profile is consumed by `draft_writer_agent` (Stage 2) and optionally by `report_compiler_agent` (Stage 1, if applicable). The Style Profile does not affect integrity verification or review stages.
...
1. **Paper writing** — delegate to `art-paper`
2. **Research** — delegate to `art-inquiry`
3. **Review** — delegate to `art-reviewer`
...
**Executor**: art-reviewer's eic_agent (Phase 2.5)
...
When `art-pipeline` mode is active, the orchestrator runs the **Cite-Time Provenance Finalizer** at every Stage 4 → Stage 5 transition (and on every revision loop pass back through Stage 4) to resolve the two-layer citation markers emitted by `synthesis_agent`, `draft_writer_agent`, and `report_compiler_agent` per Step 3a.
...
- The peer-file `<session>_human_read_log.yaml` (path computed as `<passport-path-parent>/<passport-stem>_human_read_log.yaml` per §3.6 round-5 R5-003 amend) — provides `human_read_source: true` for every `citation_key` the user has explicitly marked via `/art-mark-read`.
...
- When the joined evidence changes between passes (user acquires / verifies the source, runs `/art-mark-read <refcode>`, or runs `/art-unmark-read <refcode>` to rescind a prior mark), the next finalizer pass re-applies the matrix from the new triple and re-emits the resolved form. Promotion (e.g. `LOW-WARN` → `ok` after `/art-mark-read`) and demotion (e.g. `ok` → `LOW-WARN` after `/art-unmark-read`, since spec §3.6 line 325/340 makes the most recent timestamped event win) are both possible.
...
**Revision loops:** on revision loops (Stage 4 → reviewer → Stage 4 revise; or `art-paper` Phase 6 → Phase 4 loops), the finalizer re-runs against the current draft, resolves any newly-emitted bare `<!--ref:slug-->` comments introduced in the revision pass, and re-applies the matrix to existing resolved markers per the idempotency rule above. Resolved markers **do not invalidate** in the sense that nothing about the revision-loop mechanism itself perturbs them — only a change in the joined evidence (acquire / verify / `/art-mark-read` / `/art-unmark-read`) can move a marker. When evidence is unchanged across a revision pass, every marker is preserved byte-identical.

**LOW-WARN promotion:** when the user runs `/art-mark-read <refcode>` between finalizer passes, the next pass observes `human_read_source: true` for that slug via the read-log join and resolves the marker to row 4 (`<!--ref:slug ok-->`). The finalizer does not delete the LOW-WARN entry from the per-section checklist artifact; that artifact is informational and the user clears it manually (or it falls out at the next checklist regeneration).
...
Extends the v3.7.1 4-cell matrix above with two additive checks. External motivation: Zhao et al. arXiv:2605.07723 (2026-05). Spec: `ref/academic-research-skills/docs/design/2026-05-12-ars-v3.7.3-claim-faithfulness-and-contaminated-source-spec.md` §3.1 + §3.2.
...
**Advisory only.** The contamination annotation does NOT change the gate decision. `ok CONTAMINATED-...` still passes the formatter hard-gate; `LOW-WARN CONTAMINATED-...` is acknowledgeable via `/art-mark-read <slug>` exactly like plain LOW-WARN. The annotation surfaces the contamination signal so the user can choose to verify the source more carefully or remove the citation.
...
Spec: `ref/academic-research-skills/docs/design/2026-05-17-ars-v3.9.0-cross-index-triangulation-measurement-spec.md` §3.3.
...
**Gate semantics:** All v3.9.0 suffixes are advisory. The terminal gate refusal list is NOT extended. `formatter_agent.md` pass-through allowlist MUST extend from 3 v3.7.3 suffixes to 9 (3 legacy + 6 v3.9.0) per R-L3-2-E. `/art-mark-read` behavior is unchanged.
...
- Art-research and publishing terminology retained in English (practice-based research, ACM Reference Format, acmart/sigconf, jury review, etc.)
