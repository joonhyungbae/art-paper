---
name: art-pipeline
description: "Orchestrator for the full practice-based art-paper pipeline targeting SIGGRAPH Asia Art Papers: inquiry -> write -> integrity check -> jury review -> revise -> re-review -> re-revise -> final integrity check -> finalize (acmart PDF). Coordinates art-inquiry, art-paper, and art-reviewer into a seamless 10-stage workflow with mandatory integrity verification (artwork/realization claims + citations), two-stage jury review, and reproducible quality gates. Triggers on: art paper pipeline, research to art paper, full art-paper workflow, SIGGRAPH Asia pipeline, end-to-end art paper, practice-based publication workflow."
metadata:
  version: "0.1.0"
  last_updated: "2026-05-22"
  forked_from: "academic-research-skills academic-pipeline v3.9.4.2"
  depends_on: "art-inquiry, art-paper, art-reviewer"
  status: active
...
2. **Research integrity verification** — After paper completion and before jury review, citation faithfulness and artwork/realization claim verification must pass
...
```
I want to write an art paper about my interactive installation
```
--> art-pipeline launches, starting from Stage 1 (INQUIRY)

**Mid-entry (existing paper):**
```
I already have a paper, help me review it
```
--> art-pipeline detects mid-entry, starting from Stage 2.5 (INTEGRITY)

**Revision mode (received reviewer feedback):**
```
I received reviewer comments, help me revise
```
--> art-pipeline detects, starting from Stage 4 (REVISE)
...
- **Gate (emit)**: `CRS_PASSPORT_RESET=1` must be set in the emitting session. Without the flag, no `kind: boundary` entries are written and there is nothing to resume from.
...
**English**: art paper pipeline, research to art paper, full art-paper workflow, SIGGRAPH Asia pipeline, end-to-end art paper, practice-based publication workflow, complete art-paper workflow

### Non-Trigger Scenarios

| Scenario | Skill to Use |
|----------|-------------|
| Only need to search materials or do a literature review | `art-inquiry` |
| Only need to write a paper (no research phase needed) | `art-paper` |
| Only need to review a paper | `art-reviewer` |
| Only need to check citation format | `art-paper` (citation-check mode) |
| Only need to convert paper format | `art-paper` (format-convert mode) |
...
|-------|------|---------------------|----------------|-------------|
| 1 | INQUIRY | `art-inquiry` | socratic, full, quick | RQ Brief, Methodology, Bibliography, Synthesis |
| 2 | WRITE | `art-paper` | plan, full | Paper Draft |
| **2.5** | **INTEGRITY** | **`integrity_verification_agent`** | **pre-review** | **Integrity verification report + corrected paper** |
| 3 | REVIEW | `art-reviewer` | full (incl. Devil's Advocate) | 5 review reports + Editorial Decision + Revision Roadmap |
| 4 | REVISE | `art-paper` | revision | Revised Draft, Response to Reviewers |
| **3'** | **RE-REVIEW** | **`art-reviewer`** | **re-review** | **Verification review report: revision response checklist + residual issues** |
| **4'** | **RE-REVISE** | **`art-paper`** | **revision** | **Second revised draft (if needed)** |
| **4.5** | **FINAL INTEGRITY** | **`integrity_verification_agent`** | **final-check** | **Final verification report (must achieve 100% pass to proceed)** |
| 5 | FINALIZE | `art-paper` | format-convert | Final Paper (default MD; DOCX via Pandoc when available, otherwise conversion instructions; ask about LaTeX; confirm correctness; PDF) |
| **6** | **PROCESS SUMMARY** | **orchestrator** | **auto** | **Paper creation process record MD + LaTeX to PDF** |

**Parallelization opportunity (v3.3)**: Within Stage 2, the `art-paper` skill's Phase 1 (literature_strategist_agent) and the `visualization_agent` can operate in parallel after Phase 2 (structure_architect_agent) completes the outline. Specifically:
...
1. **Stage 1 INQUIRY** -> user confirmation -> Stage 2
...
| 3 | `integrity_verification_agent` | Integrity verifier: 100% reference/citation + artwork/realization claim verification (blocking) | `agents/integrity_verification_agent.md` |
...
1. What materials does the user have?
   - No materials           --> Stage 1 (INQUIRY)
   - Has inquiry material    --> Stage 2 (WRITE)
...
2. Trigger handoff protocol (defined in each skill's SKILL.md):
   - Stage 1  --> 2: art-inquiry handoff (RQ Brief + Bibliography + Synthesis)
   - Stage 2  --> 2.5: Pass complete paper to integrity_verification_agent
   - Stage 2.5 --> 3: Pass verified paper to reviewer
   - Stage 3  --> 4: Pass Revision Roadmap to art-paper revision mode
   - Stage 4  --> 3': Pass revised draft and Response to Reviewers to reviewer
   - Stage 3' --> 4': Pass new Revision Roadmap + R&R Traceability Matrix (Schema 11) to art-paper revision mode
...
art-pipeline is the orchestrator skill that coordinates the full art-paper pipeline across 10 stages (delegating to art-inquiry, art-paper, art-reviewer). Two invocation modes:
...
**Mode B — phase-by-phase (cross-session resume):** User invokes one phase agent at a time across sessions, typically via `CRS_PASSPORT_RESET=1` + `resume_from_passport=<hash>` (see `references/passport_as_reset_boundary.md`).

In Mode B, **single-phase agents (Bucket A per `ref/academic-research-skills/docs/design/2026-05-18-ars-v3.9.2-agent-phase-classification.md`) in the downstream skills (art-inquiry, art-paper, art-reviewer) stay strictly within their assigned phase for writes**. The 5 agents in art-pipeline itself are all cross-phase / meta by design (Bucket C/D) — they have no fence by design:
...
Stage 2.5 (pre-review) and Stage 4.5 (post-revision) verification. 5-phase protocol: references → citation context → artwork/realization claims → originality → claims. See `shared/references/art_research_evidence_model.md` §4 for the artwork/realization claim taxonomy (reception inflation, precedence/novelty, technical-capability, attribution/credit, exhibition-record).
...
- **Pipeline overrides art-paper's max 2 revision rule**: In the pipeline, revisions are limited to Stage 4 + Stage 4' (one round each), replacing art-paper's max 2 rounds rule
...
|---|---|---|
| `integrity_verification_agent` (Stages 2.5 / 4.5) | Paper content — references, citations, artwork/realization claims | Yes (blocking gate) |
...
**Cross-model**: when `CRS_CROSS_MODEL` is set, the observer runs on both models and flags any dimension divergence > 2 points. Scores are never silently averaged across models.
...
| Stage 1 | art-inquiry not converging | Suggest mode switch (socratic -> full) or narrow scope |
...
|---------|-------------|
| `examples/full_pipeline_example.md` | Complete pipeline conversation log (Stage 1-5, with integrity + 2-stage review) — interactive sound installation, practice-based art paper |
| `examples/mid_entry_example.md` | Mid-entry example starting from Stage 2.5 (existing art paper -> integrity check -> jury review -> revision -> acmart finalization) |
| `examples/integrity_failure_recovery.md` | Stage 2.5 integrity FAIL -> corrections -> re-verify PASS, on artwork/realization claims (ghost precedent ref, autonomy overclaim, reception inflation) |
...
```
art-pipeline dispatches the following skills (does not do work itself):
...
Stage 4/4': art-paper (revision mode)
Stage 5: art-paper (format-convert mode)
  - Step 1: Default to the ACM Reference Format via the `acmart` class (`sigconf` option for SIGGRAPH Asia Art Papers). Confirm the class option against the current Call for Art Papers before committing — verify against current CFP, never fabricate the required class/length/anonymization. See `shared/references/acm_reference_format.md`.
  - Step 2: Produce MD, then generate DOCX via Pandoc when available (otherwise provide conversion instructions)
  - Step 3: Produce acmart LaTeX (`\documentclass[sigconf]{acmart}`, `\bibliographystyle{ACM-Reference-Format}`) targeting the ACM `acmart` document class (the standard SIGGRAPH Asia / ACM template, obtained from CTAN or the ACM)
...
| Skill | Relationship |
|-------|-------------|
| `art-inquiry` | Dispatched (Stage 1 research phase) |
| `art-paper` | Dispatched (Stage 2 writing, Stage 4/4' revision, Stage 5 formatting) |
| `art-reviewer` | Dispatched (Stage 3 first review, Stage 3' verification review) |
...
| Dependent Skills | art-inquiry v2.0+, art-paper v2.0+, art-reviewer v1.1+ |
| Role | Practice-based art-paper workflow orchestrator |
