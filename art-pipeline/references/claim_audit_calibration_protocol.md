...
- Replace the post-calibration ramp-on plan recorded in `art-pipeline/SKILL.md` mode flags. v3.8.0 ships the audit agent dispatch as opt-in default-OFF; T-C1 passing on the canonical gold set is necessary but not sufficient evidence for default-ON.
...
|---|---|
| Stage 4 → 5 audit dispatch | Calibration runs offline; results inform the `CRS_CLAIM_AUDIT` ramp-on decision. |
...
| `/art-mark-read` clearance | Calibration does not interact with mark-read state; HIGH-WARN-CLAIM-NOT-SUPPORTED stays non-clearable regardless of calibration outcome. |
...
- **Cross-model verification**: out of scope for the calibration runner — `CRS_CROSS_MODEL` interacts with the audit agent dispatch path, not the calibration script. An operator wanting cross-model calibration runs the runner twice with different `judge_model` settings.
...
- Spec: `ref/academic-research-skills/docs/design/2026-05-15-issue-103-claim-alignment-audit-spec.md` §7.7 (test contract) + §1 deliverable 7 (this doc) + §9 (acceptance criteria) + §13 step 10 (implementation order).
- Issue: [art-paper #103](https://github.com/joonhyungbae/art-paper/issues/103) acceptance criterion (FNR < 0.15 + FPR < 0.10 gates).
- Parent agent: `art-pipeline/agents/claim_ref_alignment_audit_agent.md` — dispatches calibration mode and consumes the report's thresholds block. The agent prompt and this doc form a two-way pair following the v3.6.5 protocol-doc convention (`literature_corpus_consumers.md` ↔ `bibliography_agent.md`).
- Reviewer calibration baseline: `art-reviewer/references/calibration_mode_protocol.md` (same vocabulary, different unit of analysis).
