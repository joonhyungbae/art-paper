...
Stage 1 INQUIRY: [mode] -> [output count]
...
Stage 5 FINALIZE: Ask format style -> MD -> DOCX via Pandoc when available (otherwise instructions) -> acmart LaTeX (ACM Reference Format; verify class option against current CFP) -> tectonic -> PDF
