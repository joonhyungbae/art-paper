...
| Reuses documentation of an earlier showing of the same work, recontextualized | **Legitimate** | Re-presenting/re-reading one's own work in a new frame is legitimate — must clearly state the prior showing |
| Reuses prior work's standard description of the artist's recurring apparatus/setup | **Gray area** | A standard description of the same technical setup allows high similarity, but citing the prior write-up is recommended |
...
| **Moderate** | `MODERATE` | Individual paragraphs need rewriting | Individual paragraphs inadequately paraphrased (1-2 instances of `CLOSE_MATCH`); process/setup description overly similar to prior work |
...
| Phase C: Artwork & Realization Claim Verification | Whether artwork/realization claims are supported | C and D are complementary: C verifies claims, D verifies text |
