...
Guidelines for coordinating multi-person practice-based art-research teams using the pipeline. Claude Code runs as a single-user session; this protocol documents the **human coordination layer** that wraps around pipeline executions.
...
|------|----------------|-----------------|
| Inquiry Lead | Stage 1 (INQUIRY) | Defines the question/provocation, manages precedent search, approves synthesis, sets the conceptual framework |
| Lead Author | Stage 2 (WRITE), Stage 4/4' (REVISE) | Writes/revises paper, makes final content decisions, owns the manuscript |
| Realization Specialist | Stage 1 (making/methods), Stage 2 (realization section) | Ensures the realization account is faithful, validates technical-capability claims, reviews artwork/realization claim integrity |
...
- Inquiry Lead and Lead Author are the minimum required roles (can be the same person)
- Integration Lead is recommended for teams of 3+
- Realization Specialist is strongly recommended for technically-realized works
...
### Stage 1 -> Stage 2 (Inquiry -> Write)

| Item | Detail |
|------|--------|
| **Who hands off** | Inquiry Lead |
...
| **Approval needed** | Inquiry Lead confirms synthesis is complete and RQ is finalized |
...
| **Who receives** | Lead Author + Realization Specialist (if methodology issues) |
...
|--------------|---------------------|-----------------|
| Content disagreements | Lead Author has final say | If unresolved: Inquiry Lead mediates |
| Methodological disagreements | Realization Specialist has final say | If unresolved: cite literature precedent |
| Scope disagreements | Inquiry Lead has final say | If unresolved: team vote |
...
**From**: [Review Coordinator]
**To**: [Lead Author / Realization Specialist]
**Date**: [date]
...
```
Person A (Inquiry Lead + Lead Author):
  - Runs Stage 1 (INQUIRY) with their Claude session
  - Runs Stage 2 (WRITE) with their Claude session
  - Receives revision items, runs Stage 4/4'
  - Runs Stage 5 (FINALIZE)

Person B (Realization Specialist + Review Coordinator):
