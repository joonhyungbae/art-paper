...
**Scope**: the primary art-paper venue is **SIGGRAPH Asia Art Papers** (see `shared/references/siggraph_acm_disclosure.md` for its two-channel disclosure). The ML/NLP and science venues below are retained for art-science hybrid authors who also target an adjacent venue; more art venues are deferred to v2.
**Maintenance**: policies drift. Before submission, the user should verify against the venue's current page. The "source URL" and "access date" below record when each policy was last verified.
...
## Venue: SIGGRAPH Asia Art Papers (ACM) — primary art-paper venue

| Field | Value |
|---|---|
| Source URL | https://asia.siggraph.org/ (verify the Art Papers author guidelines against the current CFP) |
| Access date | 2026-05-23 |
| Policy summary | Two-channel disclosure: separately disclose AI used to MAKE the artwork and AI used to WRITE the paper. No AI system is an author. Authors are responsible for all content. Verify exact wording against the current CFP. |
| Required phrasing elements | Name the tool(s) and the specific role in each channel (artwork-making vs paper-writing); state the authors take responsibility for the content. |
| Preferred disclosure location | A dedicated "Use of AI Tools" statement, placed in the non-anonymized camera-ready to preserve blind review. See `shared/references/siggraph_acm_disclosure.md`. |
| Prohibited uses | Presenting AI-generated text or imagery as original without disclosure; fabricated citations or venue/exhibition records. |
| Authorship rule | No AI system can be listed as an author |
...
**Additional art venues** targeted for v2: ISEA, Leonardo (MIT Press), Ars Electronica, NIME, and art-festival catalogs. These will require separate research as their AI-disclosure policies are less standardized than the venues above.
