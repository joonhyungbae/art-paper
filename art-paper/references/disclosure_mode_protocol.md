...
4. **What art-paper did**: the mode reads the paper's commit history / pipeline log (if using the full `art-pipeline`) to identify which AI-assisted steps produced which parts of the paper. At minimum: research assistance, drafting assistance, revision assistance, citation checking, peer review simulation. If the pipeline log is not available, ask the user to confirm which categories apply.
...
| Peer review simulation | `art-reviewer` was used on the draft pre-submission |
...
- `ref/academic-research-skills/docs/design/2026-05-14-ai-disclosure-schema-decision.md` — #108 Decision Doc (G1-G10 + §4.3 invariants + §4.4 11 open concerns)
- `ref/academic-research-skills/docs/design/2026-05-14-ai-disclosure-impl-spec.md` — #108 implementation spec (resolved-paths table)
