...
│
├── Want a short concept-forward statement (single work)? ──→ artist-statement mode (Pattern 2)
├── Just need to document the work (figures + credits)? ────→ work-doc mode
├── Have an existing paper to revise? ──────────────────────→ revision mode
├── Just need format conversion (acmart LaTeX → PDF)? ──────→ format-convert mode
└── Just need a citation check (ACM + L3 locator)? ─────────→ citation-check mode
```
...
**Not Applicable When**:
- User has no idea about research direction (→ use `art-inquiry` first)
- Only need a specific section (→ use another specialized mode)

**Expected Output**: Complete paper draft + references + abstract + review report
...
- Just received materials from art-inquiry and needs to transform them into a paper plan
...
- Chapter Plan → art-reviewer (review the plan)
...
**Expected Output**: Abstract + keywords (English; art-paper v0.1 default — bilingual machinery inherited but not v0.1 default)
**Expected Duration**: Short
**Agents Used**: intake → abstract

---

### artist-statement mode — Artist Statement / Project Description (Pattern 2)

**Applicable Scenarios**:
- Need a short, concept-forward text about a single work (800-2,500 words)
- Gallery / exhibition catalog text
- A compact alternative to a full Practice-Based Art Paper

**Not Applicable When**:
- The contribution needs a full practice-based argument (→ full mode, Pattern 1)

**Expected Output**: Artist statement (provocation → the work → concept → making → significance + credits/references)
**Expected Duration**: Short
**Agents Used**: intake → structure_architect (Pattern 2) → draft_writer

---

### work-doc mode — Work Documentation

**Applicable Scenarios**:
- Need to document the work (stills, install/exhibition photos, system/process diagrams) with captions and image-credit lines
- Preparing the figure package for The Work / Realization sections

**Not Applicable When**:
- Need full prose (→ full mode)

**Expected Output**: Figure package — work stills / install photos / diagrams + captions + image credits + acmart `\includegraphics` code
**Expected Duration**: Short
**Agents Used**: intake → visualization
...
- Need an in-depth research investigation (→ art-inquiry)
...
- Markdown → acmart LaTeX → compiled PDF (default; PDF from LaTeX, IRON RULE)
- Need to comply with SIGGRAPH Asia Art Papers formatting (verify against current CFP)
...
- Final check before submission (ACM Reference Format; artwork entries = venue+date, no fabricated DOI; L3 locator gate)
- Switching citation format (e.g., APA → ACM)
...
## Paths from art-inquiry

```
art-inquiry completed
  │
  ├── art-inquiry (full mode) outputs:
  │   RQ Brief + Methodology Blueprint + Annotated Bibliography + Synthesis Report
  │   │
  │   ├── Want to write the paper directly ──→ art-paper (full mode)
  │   │   intake_agent auto-detects materials, skips redundant questions
  │   │
  │   └── Want to plan before writing ──→ art-paper (plan mode)
  │       socratic_mentor leverages existing materials to accelerate guidance
  │
  └── art-inquiry (socratic mode) outputs:
      INSIGHT Collection + Synthesis Report
      │
      ├── INSIGHTs are sufficiently clear ──→ art-paper (full mode)
      │
      └── Need more guidance ──→ art-paper (plan mode)
          socratic_mentor continues deepening from INSIGHTs
```

## Connecting to art-reviewer

```
art-paper completed
  │
  ├── full mode produces complete paper ──→ art-reviewer (full / guided)
  │   Complete peer review + revision suggestions
  │
  ├── plan mode produces Chapter Plan ──→ art-reviewer (guided)
  │   Review the plan's feasibility and completeness
  │
  └── reviewer feedback ──→ art-paper (revision mode)
...
| "Help me search for literature" / 「幫我找文獻」 | lit-review | First confirm: Is it a literature review for a paper or a research investigation? | May need art-inquiry |
| "I have art-inquiry results, help me write a paper" / 「我有研究結果，幫我寫成論文」 | full (skip Phase 0 directly) | full (but intake needs to detect handoff) | Materials need to be properly imported |
...
| art-inquiry results | Complete paper | full mode (auto-handoff) |
| art-inquiry results | Guided planning | plan mode |
