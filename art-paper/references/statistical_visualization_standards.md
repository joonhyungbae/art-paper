...
> **art-paper scope note — Pattern 5 only.** In a practice-based art paper the primary visual evidence is **work documentation** (install photos, stills, system/process diagrams with image-courtesy lines), not charts. This file applies **only** to the art-science hybrid (Pattern 5) when a genuine quantitative sub-study produces results worth charting (e.g., logged audience-interaction data, a model-behaviour metric). For ordinary art papers, do not manufacture charts; use documentation per `shared/references/art_research_evidence_model.md`. The APA 7.0 conventions below are the figure standard for such a quantitative sub-study; for an ACM/acmart venue, follow the acmart figure/caption conventions instead.

Reference guide for the `visualization_agent`. Covers figure guidelines, accessible color palettes, chart type selection, common pitfalls, and code templates for quantitative sub-studies.
