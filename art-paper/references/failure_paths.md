...
This document records the failure scenarios that the art-paper skill may encounter at each stage, their trigger conditions, and handling strategies. All agents should refer to this guide when they detect a failure scenario.
...
|---|---------|---------|--------|---------|
| F1 | Insufficient research foundation | Plan mode Step 0 finds no RQ / no data | High | Recommend running `art-inquiry` first |
...
| F6 | Poor abstract quality | Abstract omits key components or misframes the work | Medium | Re-run abstract |
...
| F9 | Incomplete handoff materials | From art-inquiry but missing key materials | Low | List missing items, suggest supplementing or re-running |
...
3. Recommend using art-inquiry (socratic mode)
4. Explain that they can come back to continue after art-inquiry is completed
...
I recommend you first use the art-inquiry tool to:
...
### F6: Poor Abstract Quality

**Trigger Timing**: Phase 5b (abstract_agent)

**Detection Indicators**:
- The abstract omits one or more of the 5 art-paper components (work / provocation / making / insight / contribution)
- Reception inflation present without an observable anchor
- Medium label dishonest (e.g., "autonomous" used for a scripted sequence)
- Word count seriously deviates from target (Pattern 1: 120-200; Pattern 5: up to 300)

**Handling Process**:
```
1. Diagnose which of the 5 components is missing or weak
2. Re-extract from the draft (the abstract must summarize what the paper actually says)
3. Rewrite to cover all 5 components within the word budget
4. Re-check reception/medium claims against the evidence model
```
...
**Trigger Timing**: intake_agent detects art-inquiry materials but they are incomplete
...
3. Recommend:
   a. Return to art-inquiry to complete the missing parts
   b. Or supplement within art-paper (add Phase 0 interview questions)
```
...
**How to Resume**: Bring this record and restart art-paper; can continue from Phase {N}
```
...
```
F1 (Insufficient research foundation) → Recommend art-inquiry → May encounter F9 (incomplete materials) upon return
...
| F6 | abstract writes independently based on the paper content as the standard |
