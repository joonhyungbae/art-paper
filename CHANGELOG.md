...
## [0.1.1] - 2026-06-13 — install/manifest hardening, naming, [worked example withheld] worked example
...
- **Stale `creative-research-skills` path in tracked config**: 23 `pdf_path` entries in `corpus_expansion/selected_corpus/cases_manifest.json` carried the pre-rename absolute path. Replaced with `<repo-root>/...`. PDF presence on the local filesystem unchanged (23 of 34 already on disk; the unresolved ones are an upstream corpus-acquisition matter, not a path bug).
...
**[worked example withheld] worked example.** Added a featured reconstruction-benchmark walkthrough on the authors' own SIGGRAPH Asia 2025 art paper, *[worked example withheld]: Playful Transgression Through VR Voice Interaction in Public Exhibition Contexts* ([authors withheld] — [affiliation withheld]). Pilot at `eval/pilot/sa25-ck/` (gitignored, consistent with the other pilots). A firewalled reconstruction agent (input-pack-only) reached a distinct reading ("the untrained voice as an embodied game controller") from the gold's withheld "playful transgression" thesis. Instrumentation: T = 0.2568 / G = 0.1261 (margin +0.1307), `thesis_supported = True`, 8-gram contamination 0.0028 (`ok`), citation precision 1.0 / recall 0.32, structural coverage 1.0.

**[worked example withheld] clean-control variant.** Added `eval/pilot/sa25-ck-clean/` with a minimal-footprint input pack — paper-derived technical specifics (RMS, YIN, Lab/Delta E*2000, multipliers, three-stage taxonomy) removed; bibliography pruned 28 → 15 to drop transgression/carnival lineage refs. A second firewalled reconstruction reached a third distinct reading ("voice as blade"). The standard margin **+0.1307 collapses to −0.0189** under the clean-control discipline (`thesis_supported = False`), an honest signature that a substantial fraction of the standard margin was an input-extraction artifact carried by paper-derived prose. Contamination stays at 0 in both runs.
...
**See-saw page removed.** `docs/{en,ko}/examples/seesaw-case.md`, all references in mkdocs nav, `nav.js`, examples/index, top-level index, getting-started, and `concepts/reconstruction-benchmark.md` removed. The [worked example withheld] case is the single featured walkthrough.
...
**Wiki deploy mechanics now documented in this changelog for the record.** The wiki source lives in two manually-synced copies — `docs/{en,ko}/` (MkDocs source in this repo) and `docs/` mirror (path withheld) (the React+Vite site under `example/the docs site`, deployed to **example.com/plugins/** via `gh-pages`). There is no sync script; changes must be mirrored to both, and the live site only updates when `npm run deploy` is run from the the docs site repo (pushing `main` is not enough). The MkDocs site under `example.com/art-paper-docs` returns 404 (private-repo + Pages-not-configured); the live surface is the docs site.
