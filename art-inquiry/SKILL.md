---
name: art-inquiry
description: "Creative-research inquiry agent team — the upstream engine for practice-based art research feeding the SIGGRAPH Asia Art Papers pipeline. 13-agent team across 7 modes: full inquiry, quick brief, work/paper review, lit-review, fact-check, Socratic guided dialogue (drawing out the artistic concept/provocation), and systematic review. Covers research-question/provocation formulation, Socratic mentoring, practice-based methodology design, art+theory literature search, source verification, cross-source synthesis, and post-research monitoring. The artwork is primary evidence (see shared/references/art_research_evidence_model.md). Triggers on: creative research, practice-based research, art research, develop my concept, position my work, art literature review, guide my art inquiry, fact-check."
metadata:
  version: "0.1.0"
  ...
  forked_from: "academic-research-skills deep-research v2.9.4"
  related_skills:
    - art-paper
    - art-pipeline
---

# Creative Inquiry — Practice-Based Art Research Agent Team

The upstream creative-research engine for practice-based art papers — a 13-agent team that helps an artist articulate the concept/provocation behind a work, position it in the art-and-technology discourse, design a practice-based methodology, and assemble the literature, treating **the artwork as primary evidence** rather than ranking empirical study designs.

Writing quality improvements in the report compiler:
- **Style Profile consumption** (optional) — If a Style Profile is available from art-paper intake, the report compiler applies it as a soft guide for the Executive Summary and Synthesis sections. Discipline conventions and report objectivity take priority.
- **Writing Quality Check** — The report compiler runs a writing quality checklist before finalizing: flags AI-typical overused terms, checks sentence/paragraph length variation, removes throat-clearing openers. See `art-paper/references/writing_quality_check.md`.
...
**Minimal command:**
```
Develop the concept behind my generative installation and position it in art-and-technology discourse
```

**Socratic mode:**
```
Guide me in articulating the artistic provocation behind my interactive work
```

**Execution:**
1. Scoping — Concept/provocation brief + practice-based methodology blueprint
2. Investigation — Art and theory literature search + source verification
3. Analysis — Cross-source synthesis (precedent works + theory) + bias check
4. Composition — Synthesis report with ACM Reference Format citations
...
**English**: creative research, practice-based research, art research, develop my concept, position my work, art literature review, precedent works, conceptual lineage, practice-based methodology, fact-check, synthesis report, guide my art inquiry, help me think through the concept, monitor this topic, set up alerts
...
"guide my art inquiry", "help me think through the concept behind my work", "draw out the provocation", or equivalent

### Does NOT Trigger

| Scenario | Use Instead |
|----------|-------------|
| Writing a paper (not researching) | `art-paper` |
...
|----------------|-----------------|----------|
| Vague idea for a work, need guidance | `socratic` | originality |
| Clear provocation, need comprehensive inquiry | `full` | balanced |
| Need a quick concept brief (30 min) | `quick` | fidelity |
| Have a paper/work to evaluate before citing | `review` | balanced |
| Need precedent-works + theory review for a topic | `lit-review` | fidelity |
| Need to verify specific claims | `fact-check` | fidelity |
| Need a systematic art-science review (Pattern 5 / hybrid) | `systematic-review` | fidelity |
...
| 3 | `bibliography_agent` | Precedent-works + theory search, source screening, annotated bibliography in ACM Reference Format | Phase 2 |
...
| 6 | `report_compiler_agent` | Drafts complete synthesis report (Title -> Abstract -> Context -> Conceptual Framework -> The Work / Realization -> Reflection -> References in ACM Reference Format) | Phase 4, 6 |
| 7 | `editor_in_chief_agent` | Editorial review (art-paper jury chair persona): originality, rigor of argument, evidence sufficiency, verdict (Accept/Revise/Reject) | Phase 5 |
...
| 10 | `socratic_mentor_agent` | Art-paper jury / mentor persona; draws out the artistic concept and provocation through Socratic questioning across 5 layers | Socratic Mode (Layer 1-5) |
...
| 14 | `timeline_extraction_agent` | Extracts per-source temporal facts and citation provenance into Phase 2 sidecar artifacts (`timeline.yaml`, `citation_provenance.yaml`) for the temporal-verification gate | Phase 2 |
...
=== Phase 2: INVESTIGATION ===
     ...
     |   - PRISMA-style screening trail (if applicable)
     |   - Annotated bibliography (ACM Reference Format; precedent works use venue/date locators)
     |
...
=== Phase 4: COMPOSITION ===
     |
     +-> [report_compiler_agent] -> Full Synthesis Report Draft
         - Title
         - Abstract (120-200 words)
         - Introduction / Context (the work, artistic context, the provocation, contribution)
         - Conceptual Framework / Theory + precedent-works positioning
         - The Work + Realization / Methods of Making
         - Reflection / Discussion (situated insight, exhibition & reception, limitations)
         - Conclusion & Future Work
         - References (ACM Reference Format)
         - Appendices / documentation (if applicable)
         - (default structure: Practice-Based Art Paper, Pattern 1; see shared/references/art_paper_structure_patterns.md)
     |
...
art-paper pipeline runs in 6 phases. Two invocation modes:
...
**Mode B — phase-by-phase (cross-session resume):** User invokes one agent per phase across sessions for long-running projects. Common pattern via `CRS_PASSPORT_RESET=1` + `resume_from_passport=<hash>` (see `art-pipeline/references/passport_as_reset_boundary.md`).

In Mode B, **single-phase agents (Bucket A per `ref/academic-research-skills/docs/design/2026-05-18-ars-v3.9.2-agent-phase-classification.md`) stay strictly within their assigned phase for writes**. Reads from upstream phases are allowed. Multi-phase agents (Bucket B: `devils_advocate_agent`, `report_compiler_agent`) do exactly the work specified by the caller's invocation for that phase — no extension to other phases in the same call.
...
|------|---------------|--------|------------|
| `full` (default) | All 9 core (excluding socratic_mentor, RoB, meta-analysis) | Full synthesis report (ACM Reference Format) | 3,000-8,000 |
...
| Only non-English sources available | English search returns empty | Note language coverage as a limitation; broaden to non-English art/theory sources where the user reads them |
...
After research is complete, the following materials can be handed off to `art-paper`:
...
`art-paper`'s `intake_agent` will automatically detect available materials and skip redundant steps:
...
| `references/irb_decision_tree.md` | Art-research ethics decision tree (consent/representation, image & exhibition rights, courtesy lines; Pattern-5 audience-study review) | ethics_review, research_architect |
...
|---------|-------------|
| `examples/exploratory_research.md` | Full 6-phase pipeline walkthrough (generative air-quality installation) |
| `examples/systematic_review.md` | Precedent-works + theory review (biometric-input installations) |
| `examples/policy_analysis.md` | Comparative lineage study (authorship across generative artworks) |
| `examples/socratic_guided_research.md` | Complete Socratic mode multi-turn dialogue drawing out an artistic provocation (12 rounds) |
| `examples/handoff_to_paper.md` | art-inquiry full mode handoff to art-paper |
| `examples/review_mode.md` | Review mode: 3-agent review of an art-paper contribution + reception passage |
| `examples/fact_check_mode.md` | Fact-check mode: per-claim verification of art-history / precedence / exhibition claims |
...
1. ⚠️ **IRON RULE**: **Every claim must be anchored to an evidence type** — no unsupported assertions (work / process / exhibition+reception / lineage / reflection; see `shared/references/art_research_evidence_model.md`)
2. **Evidence triangulation, not ranking** — the artwork is primary evidence; corroborate across work, process, exhibition/reception, conceptual lineage, and situated reflection rather than ranking study designs
3. **Contradiction disclosure** — if sources disagree, report both sides with a comparison of how well each is evidenced
...
This skill can be combined with other art-paper skills:

```
art-inquiry + art-paper          -> Full concept-to-publication pipeline
art-inquiry (socratic) + art-paper (plan) -> Guided concept dialogue + paper planning
art-inquiry (systematic-review) + art-paper -> Art-science hybrid (Pattern 5) review paper
art-inquiry + art-reviewer       -> Inquiry then jury review of the work/paper
```
...
| Dependent Skills | art-paper v1.0+ (downstream) |
