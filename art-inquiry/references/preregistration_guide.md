...
> **art-paper scope note — Pattern-5 / systematic-review only.** Preregistration is a confirmatory-empirical transparency tool. In art-paper it applies ONLY to the empirical sub-question of an art-science hybrid (Pattern 5) or to `systematic-review` mode (PROSPERO registration). Practice-based / practice-led inquiry is NOT preregistered — making is exploratory and situated by nature (see `shared/references/art_research_evidence_model.md`). The examples below are inherited illustrations of the mechanism, not art-paper defaults.
...
## 4. Pattern-5 (Art-Science Hybrid) Preregistration Examples

> These illustrate preregistration of the EMPIRICAL sub-question of an art-science hybrid — e.g., a perception/HCI study attached to a media-art work. The artwork itself is not preregistered; only the empirical study is.

### Example: Audience-Perception Study Attached to a Media-Art Installation

```
Title: Does Haptic Feedback Increase Perceived Immersion in an Interactive
       Installation? A Preregistered Within-Subjects Study

Hypotheses:
H1: Visitors will report significantly higher perceived immersion in the
    haptic-enabled condition than in the visual-only condition
H2: The effect will be larger for visitors with prior VR/installation
    experience

Design: Within-subjects, order counterbalanced (haptic vs. visual-only)
Sample: 60 gallery visitors who opt in at the venue
Power: 80% power to detect dz = 0.4, alpha = .05

Primary outcome: Immersion questionnaire score (validated scale)
Secondary outcomes: Dwell time, self-reported emotional intensity
Analysis: Paired-samples comparison; mixed model with experience covariate
...
- Prior-experience × condition interaction
- Dwell time as a behavioural correlate of self-reported immersion
```

### Example: Systematic Review Behind a Media-Art Work's Empirical Claim

```
Title: Audience Presence in Interactive Installations: A Systematic Review
       of HCI Evidence

Research question: What does HCI evidence say about how interactivity shapes
                   audience presence/engagement in installation settings?
Databases: ACM Digital Library, IEEE Xplore, Scopus, Web of Science
Search strategy: (interactiv*) AND (installation OR "media art" OR exhibit*)
                 AND (presence OR engagement OR immersion)
Time range: 2005-2025
Inclusion criteria:
- Empirical studies of audience response in interactive/installation settings
- Peer-reviewed conference papers or journal articles
Exclusion criteria:
- Pure system/technical papers with no audience measurement
- Opinion or curatorial essays (no empirical data)
