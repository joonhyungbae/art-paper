...
> **art-paper genre note.** For practice-based art research, the load-bearing ethics dimensions are **copyright & exhibition/display rights, collaboration credit, representation, and two-channel AI disclosure** (paper-making vs artwork-making). See `ethics_review_agent.md`, `shared/references/creative_art_terminology_glossary.md` (§4 credit, §5 rights), and `shared/references/siggraph_acm_disclosure.md` — these are canonical for the genre. The human-subjects / IRB sections below apply ONLY when the inquiry observes or records audiences (e.g., an art-science hybrid user study); simply exhibiting a work to an audience is NOT human-subjects research.

## Purpose
Comprehensive ethics checklist for AI-assisted research. Used by the ethics_review_agent alongside the art-ethics references above.
...
- [ ] Scope of AI involvement specified:
  ...
  - [ ] Data analysis
  - [ ] Translation
- [ ] Human oversight described (who reviewed what, at which stages); authors take full responsibility
- [ ] AI limitations acknowledged (potential hallucination, knowledge cutoff, etc.)
- [ ] AI version/date noted (for reproducibility)
- [ ] **Two-channel distinction (art-paper specific)**: AI used to *make the artwork* (a medium/process of the work) is disclosed separately in §Realization, distinct from AI used to *write the paper*. No AI system is listed as an author. See `shared/references/siggraph_acm_disclosure.md`.
...
- [ ] Remove direct identifiers (names, contact details, national ID numbers)
- [ ] Assess indirect identifier risks (small-venue + role + demographic combinations)
- [ ] Small-N re-identification risk assessment (e.g., a small exhibition audience may allow re-identification of individuals)
...
| **Participants in a power relationship (e.g., the artist's own students or collaborators)** | Avoid power dynamics affecting voluntariness, use third-party recruitment |
...
> For the art-research ethics decision tree (consent/representation, rights & courtesy lines, and the audience-study / art-science-hybrid review path): see `references/irb_decision_tree.md`
