...
> **art-paper scope note — Pattern-5 / systematic-review only.** EQUATOR reporting checklists are an empirical-science apparatus. In art-paper they apply ONLY to the empirical sub-question of an art-science hybrid (Pattern 5) or to `systematic-review` mode. They do NOT apply to the default practice-based art paper, whose evidence model is `shared/references/art_research_evidence_model.md` (the artwork is primary evidence; claims are triangulated, not ranked). The higher-education examples below are inherited illustrations of the guidelines themselves, not art-paper defaults.
