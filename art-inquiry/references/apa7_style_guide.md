...
> **art-paper status — NON-DEFAULT (superseded for the art-research genre).** The default citation format for art-paper is the **ACM Reference Format** (`shared/references/acm_reference_format.md`), required by SIGGRAPH Asia Art Papers (proceedings on the ACM Digital Library; verify against the current CFP). `report_compiler_agent`, `bibliography_agent`, and `editor_in_chief_agent` default to ACM. This APA guide is retained only as a non-default fallback (e.g., a non-ACM target venue). Verify the required format against the current Call for Art Papers.

## Purpose
Quick reference for APA 7.0 formatting. NON-DEFAULT — see the ACM Reference Format banner above.
